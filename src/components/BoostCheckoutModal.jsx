...
import { initiateBoostPayment, fetchBoostStatus } from '../services/subscriptions';
...
export const BoostCheckoutModal = ({ 
  ...
  const [budget, setBudget] = useState(budgetFcfa || 5000);
  const [duration, setDuration] = useState(dureeJours || 7);
  ...
  const startBoostStatusPolling = (boostId) => {
    ...
    pollTimerRef.current = setInterval(async () => {
      attempts += 1;
      const boostData = await fetchBoostStatus(boostId);
      if (boostData?.statut === 'actif') {
        clearInterval(pollTimerRef.current);
        setPollingStatus('success');
        setLoading(false);
        if (onSuccess) onSuccess(boostData);
      } else if (boostData?.statut === 'annule' || boostData?.statut === 'expire') {
        clearInterval(pollTimerRef.current);
        setPollingStatus('failed');
        setLoading(false);
        setPaymentLocked(false);
        setApiError('Le paiement du boost de visibilité n\'a pas pu être validé.');
      } else if (attempts >= maxAttempts) {
        clearInterval(pollTimerRef.current);
        setPollingStatus('failed');
        setLoading(false);
        setPaymentLocked(false);
        setApiError('Délai d\'attente dépassé pour la confirmation du boost.');
      }
    }, 4000);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await initiateBoostPayment({
        ...
        budget_fcfa: Number(budget),
        duree_jours: Number(duration),
      ...
      } else if (response?.boost_id) {
        startBoostStatusPolling(response.boost_id);
      } else {
        setLoading(false);
      ...
      if (msg.includes('502') || msg.toLowerCase().includes('passerelle') || msg.toLowerCase().includes('gateway')) {
        ...
        setApiError('La passerelle de paiement rencontre un souci technique (502). Vous pouvez réactiver votre boost via WhatsApp.');
      } else {
        setApiError(msg);
      }
    }
      ...
      <div className="bg-[#0F2318] border border-white/10 text-white rounded-3xl max-w-md w-full p-6 relative shadow-2xl space-y-5 overflow-y-auto max-h-[90vh]">
        ...
        {pollingStatus === 'polling' ? (
          <div className="py-8 text-center space-y-4">
            <div className="w-14 h-14 rounded-2xl bg-amber-500/20 border border-amber-500/30 flex items-center justify-center text-amber-400 mx-auto animate-pulse">
              <IconLoader2 size={32} className="animate-spin" />
            </div>
            <div>
              <h4 className="font-bold text-lg text-white">Validation du boost en cours...</h4>
              <p className="text-xs text-white/60 mt-1 max-w-xs mx-auto">
                Veuillez valider la transaction sur votre téléphone ({telephone}). Votre terrain sera propulsé dès confirmation.
              </p>
            </div>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="pt-2 space-y-3">
              <button
                type="submit"
                disabled={loading || paymentLocked || !telephone || !selectedTerrainId}
                className="w-full py-4 bg-amber-500 hover:bg-amber-600 disabled:bg-gray-600/40 text-black font-bold rounded-2xl text-sm transition-all shadow-lg flex items-center justify-center gap-2 cursor-pointer disabled:cursor-not-allowed"
              >
                {loading ? (
                  <>
