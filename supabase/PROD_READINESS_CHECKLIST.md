...
- [x] **Vérifié (2026-07-31)** : `curl -i "https://api.unitech.sn/api?action=balance"` et `curl -i "https://api.unitech.sn/api.php?action=balance"` répondent tous les deux `200 OK` avec exactement le même payload (`{"success":true,"data":{"sold_wave":0,"sold_om":0,"sold_intl":0,"total":0,"currency":"XOF"}}`) avec la clé API réelle — le serveur route visiblement les deux chemins vers le même script. La clé API est valide et fonctionnelle. **Aucun changement de code nécessaire** : `/api.php` (utilisé partout dans le code actuel) fonctionne bien en production, ce n'est pas la cause du problème initial.
- [x] **Corrigé (2026-07-27)** : `UNITECH_WEBHOOK_SECRET` est maintenant le seul nom utilisé partout (`api/index.js`, `backend/server.js`, `backend/routes/webhooks.js` — ce dernier lisait `UNITECHPAY_WEBHOOK_SECRET`). Les 3 handlers rejettent désormais la requête (fail-closed) si le secret n'est pas configuré ou si la signature est manquante/invalide — au lieu de sauter silencieusement la vérification. `backend/server.js` avait en plus un bug distinct : sa branche `reference.startsWith('PS-')` (le préfixe réellement généré par `backend/routes/payments.js` pour les paiements de réservation) `return`ait AVANT même d'atteindre le check de signature plus bas dans le fichier — contournement total, secret configuré ou non. La vérification est maintenant déplacée avant cette branche.
  - [ ] **Reste à faire (toi)** : définir `UNITECH_WEBHOOK_SECRET` avec la vraie valeur fournie par UnitechPay, dans les variables d'environnement de la plateforme qui sert réellement la prod (Vercel pour `api/index.js`, cf. item ci-dessous). Sans ça, le webhook rejette maintenant TOUT le trafic (fail-closed) — la config n'est plus optionnelle, elle est requise pour que les paiements de réservation confirment quoi que ce soit.
  - [ ] Vérifier côté dashboard/doc UnitechPay le nom exact de l'en-tête de signature attendu et l'algorithme (ce code suppose HMAC-SHA256 sur `x-unitechpay-signature`, calculé sur `JSON.stringify({reference, status})` pour `api/index.js`/`server.js`, et sur le corps brut pour `routes/webhooks.js` — ces deux calculs ne signent pas la même chose, à réconcilier avec UnitechPay si les deux endpoints doivent un jour être actifs simultanément).
- [x] **Fait (2026-07-31)** : webhook UnitechPay enregistré via `configure_webhook` (API réelle, clé de prod) pointant vers `https://ahqtcgxrewrfbowblygu.supabase.co/functions/v1/webhook-unitech` — réponse `{"success":true,"data":{"message":"Webhook configuré avec succès"}}`. C'est désormais la SEULE URL que le compte marchand notifiera.
  - [ ] Reste à faire : déployer `webhook-unitech` (pour que `supabase/config.toml` → `verify_jwt=false` prenne effet) et définir `UNITECH_WEBHOOK_SECRET` (= même valeur que `UNITECH_API_KEY`) dans les secrets Edge Functions du dashboard Supabase.
